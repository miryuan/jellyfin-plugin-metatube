        ...
        IHasOrder
{
    ...
    public ActorProvider(ILogManager logManager)
        : base(logManager.CreateLogger<ActorProvider>())
#else
    public ActorProvider(ILogger<ActorProvider> logger)
        : base(logger)
#endif
    { }
    ...
    )
    {
        ...
        foreach (var m in searchResults)
        {
            var result = new RemoteSearchResult
            {
                ...
                ImageUrl =
                    m.Images?.Any() == true
                        ? ApiClient.GetPrimaryImageApiUrl(
                            m.Provider,
                            m.Id,
                            m.Images.First(),
                            0.5,
    ...
    private static string FormatOverview(ActorInfo a)
    {
        var aliases = a
            .Aliases?.Where(alias =>
                !string.Equals(alias, a.Name, StringComparison.OrdinalIgnoreCase)
            )
        ...
        return string.Join(
            "\n<br>\n",
            info.Where(kvp => !string.IsNullOrWhiteSpace(kvp.Item2))
