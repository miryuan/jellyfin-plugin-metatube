...
#endif
{
    ...
    private static readonly string[] AvBaseSupportedProviderNames =
    {
        "DUGA",
        "FANZA",
        "Getchu",
...
#if __EMBY__
    public MetadataFeatures[] Features =>
        new[]
        {
    ...
    )
    {
        ...
        m.Genres =
            ...
            ?? Array.Empty<string>();
        m.Actors =
            m.Actors?.Where(x => !string.IsNullOrWhiteSpace(x)).Distinct().ToArray()
            ?? Array.Empty<string>();
        m.PreviewImages =
            ...
            Item = new Movie
            {
                Name = RenderTemplate(
                    Configuration.EnableTemplate
                        ...
                        : PluginConfiguration.DefaultNameTemplate,
                    parameters
                ),
                Tagline = RenderTemplate(
                    Configuration.EnableTemplate
                        ? Configuration.TaglineTemplate
                        : PluginConfiguration.DefaultTaglineTemplate,
                    parameters
                ),
        ...
        if (Configuration.EnableMovieProviderFilter)
        {
            if (Configuration.GetMovieProviderFilter() is { } filter && filter.Any()) // Apply only if filter is not empty.
            {
                ...
                searchResults = searchResults
                    .OrderBy(m =>
                        filter.FindIndex(s =>
                            s.Equals(m.Provider, StringComparison.OrdinalIgnoreCase)
                        )
                    )
                    .ToList();
            }
    ...
    private async Task ConvertToRealActorNames(
        MovieSearchResult m,
        CancellationToken cancellationToken
    )
    {
        if (!AvBaseSupportedProviderNames.Contains(m.Provider, StringComparer.OrdinalIgnoreCase))
            return;
        ...
        try
        {
            ...
            foreach (var result in searchResults)
            {
                ...
                Logger.Info(
                    "Calculate movie title similarity for {0} ({1}) and {2} ({3}): {4:0.00%}",
                    m.Id,
                    m.Provider,
                    result.Id,
                    result.Provider,
    ...
    private static double CalculateTitleSimilarity(
        MovieSearchResult source,
        MovieSearchResult target
    )
    {
    ...
    private async Task TranslateMovieInfo(
    ...
    private static string RenderTemplate(string template, Dictionary<string, string> parameters)
    {
        ...
        var sb = parameters
            .Where(kvp => template.Contains(kvp.Key))
            .Aggregate(new StringBuilder(template), (sb, kvp) => sb.Replace(kvp.Key, kvp.Value));
